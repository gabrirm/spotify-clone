...
const createOrRetrieveCustomer = async ({
  ...
  if (error || !data?.stripe_customer_id) {
    ...
    if (supabaseError) {
      console.log("Error creating customer", supabaseError);
      throw supabaseError;
    }
...
const manageSubscriptionStatusChange = async (
  ...
  if (noCustomerError) {
    console.log("Error retrieving customer", noCustomerError);
  ...
  if (error) {
    console.log(
      `Error upserting subscription [${subscription.id}] for ${uuid}`
    );
    throw error;
  }
  console.log("Upserted subscription", subscription.id);
